    ...
    <div class="py-5 px-5">
        ...
        <div class="text-right">
            <a href="{{ route('category#createPage') }}"
                ...
                <span class="text-sm">Add category</span></a>
        </div>
    </div>
    <div class="py-5 px-5">
        ...
        <table class="table-auto w-full text-center ">
            <thead class="bg-indigo-500  text-white">
                <tr>
                    ...
                    <th class=" py-2">No</th>
                    <th class=" py-2 w-6/12">Categories</th>
            ...
            <tbody>
                @foreach ($category as $c)
                    <tr class="border-b">
                        ...
                        <td class=" py-2 id">{{ $c->id }}</td>
                        <td class=" py-2">{{ $c->name }}</td>
                        <td class=" py-2">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" value="{{ $c->publish }}" @checked($c->publish == 1)
...
@section('scriptSource')
    <script>
        $(document).ready(function() {
            $('#category').addClass('bg-indigo-500 text-white hover:text-slate-50')
            $('#category path').css({
                "stroke": "white",
            })
            ...
            $('.delete-btn').on('click', function() {
                $itemId = $(this).closest('tr').find('.id').text() * 1;
